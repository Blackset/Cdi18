Gérer une liste de noms classés alphabétiquement.  (page 94)
...
Types chaînenom = tableau [ tailnom ] de caractère     
	...
	tablenom = Enregistrement                                                    
			...
			table : tableau [ tailtable ] de élément     // table permet de ranger les informations       tablenom.table[  ].nom    tablenom.table[  ].suivant   
				...
				SINON
					...
					ON modifie libre
						...
						SINON
										...
										// indice = final
								(milieu ou dernier)
								tablenom (nouvel-indice) = indice-nom (nom-avant)
								indice (tablenom (nom-avant)) = nouvelle-indice
						FINSI
				-->AJOUT
				FINSI
		FINSI
...
// soit le mot existe déjà alors on ne fait rien et on retourne  un code-retour	--> "rien à faire"
// soit la table est pleine et on retourne un code-retour-->						"table pleine"
// soit on ajoute le prénom et on retourne 	un code-retour-->						"ajout"
// tablenom est modifié en fonction de l'opération effectuée

Procédure ajouter ( Entrée Sortie tablenom : table, Entrée nom : chaînenom ) : chaine 
 ...
 Sinon  	// recherche ou ranger le nom dans le tableau   
	   ...
	   // sa place entre nom-avant et indice-nom ou en fin de liste     
		...
		Tantque    ( indice-nom <> final ) et ( tablenom.table [ indice-nom ].nom < nom )    Faire    
							// quand fin de liste ou quand on trouve un prénom >= dans la table  soit le suivant 
							// on connait le nom suivant  // nom suivant dans la table
			nom-avant := indice-nom    // et avant
		...
		Sinon // rangement du prénom dans la table des noms  
				...
				// soit on insère le prénom en premier soit dans la liste  
		 
						   Si    nom-avant = final    Alors      // soit c’est le premier élément de la table de prénoms       
								   // soit nom est le plus petit prénom de la table donc premier       
							...
							Sinon      
								   ...
								   tablenom.table [ nouvel-indice ] := indice-nom    // (0 à 100)
						   Finsi 
			...
			Chercher PRENOM à enlever 
				...
				VALEUR TABLE
					...
					SINON
						...
						( milieu ou dernier) 
						indice (tablenom (NON-AVANT)) --> indice (NON SUIVANT)
					FINSI
					ON modifie libre
				-->ENLEVE
				FINSI
...
Procédure enlever ( Entrée Sortie tablenom : table, Entrée nom : chaînenom ) : chaine 
...
Variables 	code-retour  : chaine // code-retour est le code de retour de la fonction    
			...
			indice-nom : entier // indice-nom est l’indice du nom inspecté
	  ...
	  indice-nom := tablenom.premier 	 
	   // parcours de la liste des noms jusqu’à trouver le nom à l’indice indice-nom,  
					...
					Tantque    ( indice-nom <> final ) et ( tablenom.table [ indice-nom].nom < nom )    Faire   // nom contenu dans la table  
								  // arrêt en fin de liste ou nom dans la table  
						nom-avant := indice-nom    // non avant
						indice-nom := tablenom.table[ indice-nom ].suivant    // nom suivant dans la table
					Fintantque 
	 
									// si trouvé on a non avant 
